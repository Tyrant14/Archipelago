...
from .options import RandomizeWildPokemon, RandomizeTrainerParties, EliteFourRequirement, CyrusRequirement
...
location_visited_event_to_id_map = {
    ...
    "EVENT_VISITED_SANDGEM_TOWN": 1,
    "EVENT_VISITED_JUBILIFE_CITY": 2,
    "EVENT_VISITED_OREBURGH_CITY": 3,
    "EVENT_VISITED_FLOAROMA_TOWN": 4,
    "EVENT_VISITED_ETERNA_CITY": 5,
    "EVENT_VISITED_HEARTHOME_CITY": 6,
    "EVENT_VISITED_SOLACEON_TOWN": 7,
    "EVENT_VISITED_VEILSTONE_CITY": 8,
    "EVENT_VISITED_PASTORIA_CITY": 9,
    "EVENT_VISITED_CELESTIC_TOWN": 10,
    "EVENT_VISITED_CANALAVE_CITY": 11,
    "EVENT_VISITED_SNOWPOINT_CITY": 12,
    "EVENT_VISITED_SUNYSHORE_CITY": 13,
    "EVENT_VISITED_POKEMON_LEAGUE": 14,
    "EVENT_VISITED_FIGHT_AREA": 15,
    "EVENT_VISITED_RESORT_AREA": 16,
    "EVENT_VISITED_SURVIVAL_AREA": 17
}
...
def generate_output(world: "PokemonPlatinumWorld", output_directory: str) -> None:
    ...
    for location in world.multiworld.get_locations(world.player):
        ...
        if location.address is None:
            if world.options.free_fly_location and location.name == "EVENT_VISITED_TWINLEAF_TOWN":
    ...
    starting_badges = 0
    if start_inventory.pop("Coal Badge", 0) > 0:
        starting_badges |= (1 << 0)
    if start_inventory.pop("Forest Badge", 0) > 0:
        starting_badges |= (1 << 1)
    if start_inventory.pop("Relic Badge", 0) > 0:
        starting_badges |= (1 << 2)
    if start_inventory.pop("Cobble Badge", 0) > 0:
        starting_badges |= (1 << 3)
    if start_inventory.pop("Fen Badge", 0) > 0:
        starting_badges |= (1 << 4)
    if start_inventory.pop("Mine Badge", 0) > 0:
        starting_badges |= (1 << 5)
    if start_inventory.pop("Icicle Badge", 0) > 0:
        starting_badges |= (1 << 6)
    if start_inventory.pop("Beacon Badge", 0) > 0:
        starting_badges |= (1 << 7)
    ...
    #     /* 0x0E */ bool8 cyrusRequiresGyms;
    #     /* 0x0F */ u8 cyrusRequiredCount;
    ...
    # Set cyrus requirement
    cyrus_requires_gyms = 1 if world.options.cyrus_requirement == CyrusRequirement.option_gyms else 0
    _set_bytes_little_endian(patched_rom, options_address + 0x0E, 1, cyrus_requires_gyms)

    # Set cyrus count
    cyrus_count = min(max(world.options.cyrus_count.value, 0), 8)
    _set_bytes_little_endian(patched_rom, options_address + 0x0F, 1, cyrus_count)
    ...
    removed_roadblocks_bitfield |= (1 << 0) if "Jubilife City Poketch" in removed_roadblocks else 0
    removed_roadblocks_bitfield |= (1 << 1) if "Eterna Forest Galactic Grunts" in removed_roadblocks else 0
    removed_roadblocks_bitfield |= (1 << 2) if "Solaceon Town Psyducks" in removed_roadblocks else 0
    removed_roadblocks_bitfield |= (1 << 3) if "Route 222 Lady" in removed_roadblocks else 0
    removed_roadblocks_bitfield |= (1 << 4) if "Canalave City Battlers" in removed_roadblocks else 0
    ...
    output_path = os.path.join(output_directory, f"{out_file_name}.nds")
